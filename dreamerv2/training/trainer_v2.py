...
from dreamerv2.models.rssm_v2 import RSSM
# from dreamerv2.models.pixel_v2 import ObsDecoder, ObsEncoder
...
class Trainer(object):
    ...
    def train_batch(self, train_metrics):
        ...
        if self.config.harmony:
            train_metrics['harmonizer_rec'] = self.harmonizer_rec.get_harmony().item()
            train_metrics['harmonizer_rew'] = self.harmonizer_rew.get_harmony().item()
            train_metrics['harmonizer_kl'] = self.harmonizer_kl.get_harmony().item()
    ...
    def actorcritc_loss(self, posterior):
        ...
        # there's no DiscountModel freezing in ifactor
    ...
    def representation_loss(self, obs, actions, rewards, nonterms):
        ...
        if self.config.harmony:
            div = self.harmonizer_kl(div)
            reward_loss = self.harmonizer_rew(reward_loss)
            obs_loss = self.harmonizer_rec(obs_loss)
            pcont_loss *= self.loss_scale['discount']
        else:
            div *= self.loss_scale['kl']
            reward_loss *= self.loss_scale['reward']
            pcont_loss *= self.loss_scale['discount']

        model_loss = div + reward_loss + obs_loss + pcont_loss
    ...
    def _actor_loss(self, imag_reward, imag_value, discount_arr, imag_log_prob, policy_entropy):
        ...
        # discount_arr = torch.cat([discount_arr[:1], discount_arr[1:]])  # ifactor
    ...
    def _value_loss(self, imag_modelstates, discount, lambda_returns):
        ...
        """critic_loss = -critic(state).log_prob(lambda_return)"""
    ...
    def _model_initialize(self, config):
        ...
        deter_size = config.rssm_info['deter_size']  # 400
        if config.rssm_type == 'continuous':
            stoch_size = config.rssm_info['stoch_size']  # 20
        elif config.rssm_type == 'discrete':
            category_size = config.rssm_info['category_size']  # 20
            class_size = config.rssm_info['class_size']  # 20
            stoch_size = category_size*class_size  # 400
        ...
        if self.config.harmony:
            self.harmonizer_rec = Harmonizer(self.device)
            self.harmonizer_rew = Harmonizer(self.device)
            self.harmonizer_kl = Harmonizer(self.device)
    ...
    def _optim_initialize(self, config):
        ...
        if self.config.harmony:
            self.world_list += [self.harmonizer_rec, self.harmonizer_rew, self.harmonizer_kl]
